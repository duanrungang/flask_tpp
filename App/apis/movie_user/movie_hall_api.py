...
class UserMovieHallsResource(Resource):
    def get(self):
        ...
        """
            ...
            hall_id
        """
        cinema_address = CinemaAddress.query.filter(
            CinemaAddress.district == district
        ).filter(
            CinemaAddress.id == address_id
        ).first()
...
class UserMovieHallResource(Resource):
    ...
    def get(self, id):
        ...
        movie_orders_lock = MovieOrder.query.filter(MovieOrder.o_hall_movie_id == id).filter(MovieOrder.o_status == ORDER_STATUS_NOT_PAY).filter(
