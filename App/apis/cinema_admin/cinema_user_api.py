...
from App.apis.utils import generate_cinema_user_token
...
parse_register.add_argument('username', type=str, required=True, help='请输入姓名：')
parse_register.add_argument('phone', type=str, required=True, help='请输入手机号：')

parse_login = parse_base.copy()
parse_login.add_argument('username', type=str, help='请输入姓名：')
...
class CinemaUsersResource(Resource):
    def post(self):
        ...
        if action == USER_ACTION_REGISTER:
            ...
            cinema = CinemaUser()
            cinema.username = username
            cinema.password = password
            cinema.phone = phone
